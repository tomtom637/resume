...
function translate() {
  if (!translationState) {
    for (const property in translations) {
      originalTranslations[property] = document.querySelector(`[data-translation="${property}"]`).innerHTML;
    ...
    for (const property in originalTranslations) {
      document.querySelector(`[data-translation="${property}"]`).innerHTML = originalTranslations[property];
    }
